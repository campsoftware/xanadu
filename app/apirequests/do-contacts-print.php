...
$recs->querySQL = 'SELECT * FROM ' . $mmAPIRequestsT->NameTable . ' WHERE ' . $mmAPIRequestsT->NameTableKey . ' = ?';
$recs->queryBindNamesA = array( $mmAPIRequestsT->NameTableKey );
$recs->queryBindValuesA = array( $doParam[ $mmAPIRequestsT->NameTableParam ] );
$recs->query();
$recs->rowsMassageForGUI( true );

// Error Check
if ( $recs->errorB ) {
	$ValidationMsgA[] = $mmAPIRequestsT->NameSingular . ' Print Error' . $recs->messageExtra . '; ' . $recs->messageSQL;
...
$recsComms->querySQL = 'SELECT * FROM ' . $mmContactsCommsT->NameTable . ' WHERE ' . $mmAPIRequestsT->NameTableKey . ' = ?';
$recsComms->queryBindNamesA = array( $mmAPIRequestsT->NameTableKey );
$recsComms->queryBindValuesA = array( $doParam[ $mmAPIRequestsT->NameTableParam ] );
$recsComms->query();
$recsComms->rowsMassageForGUI( true );

// Error Check
if ( $recsComms->errorB ) {
	$ValidationMsgA[] = $mmAPIRequestsT->NameSingular . ' Print Error' . $recsComms->messageExtra . '; ' . $recsComms->messageSQL;
...
foreach ( $recs->rowsD as $recsRow ) : ?>
    ...
    <div class="floater">
        ...
        <table class="tableBorderNone">
            <tr>
                <td>
                    <img class="cellBorder" src='<?= \xan\fileBucketURL( $mmAPIRequestsT->NameTable, $recsRow[ $mmAPIRequestsT->NameTableKey ], 'PhotoFN', $recsRow[ 'PhotoFN' ] ) ?>' style="max-width: auto; max-height: 12rem;" alt="Photo">
                </td>
            </tr>
        </table>
    </div>
...
$docTitle = 'Contact - ' . $mmAPIRequestsT->getDisplayName( $recs );
