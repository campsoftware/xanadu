...
if ( xan\isNotEmpty( $resp->reqID ) ) {
	...
	if ( $recsDetail->errorB ) {
		...
		// Detail Cards Append
		require_once( 'content-card-usersUser.php' );
		require_once( 'content-card-usersPrivs.php' );
		require_once( 'content-card-usersAuth.php' );
		require_once( 'content-card-usersMeta.php' );
		...
		<div class="float-right">
			<?php if ( xan\isNotEmpty( $resp->reqID ) ) : ?>
				...
				<div class="dropdown-menu dropdown-menu-right border-1" role="menu" aria-labelledby="actionsMenu">
					<a class="dropdown-item pl-2 pr-2" onclick='xanDo( { "Msg":"Printing", "URL":"<?= $mmUsersT->URLDoRelative ?>", "Do":"UsersPrint", "IDUsers":"<?= $resp->reqID ?>", "Format":"pdf", "Template":"pdf-default", "NewWindow":1 } );'><?= FA_PRINT . STR_NBSP ?>Print to PDF</a>
					<a class="dropdown-item pl-2 pr-2" onclick='xanDo( { "Msg":"Printing", "URL":"<?= $mmUsersT->URLDoRelative ?>", "Do":"UsersPrint", "IDUsers":"<?= $resp->reqID ?>", "Format":"html", "Template":"pdf-default", "NewWindow":1 } );'><?= FA_PRINT . STR_NBSP ?>Print to HTML</a>
